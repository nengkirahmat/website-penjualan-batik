...
if (!empty($_GET['action']) and $_GET['action']=="status_bayar" and !empty($_GET['id_trans'])){
	...
	$res=$con->query($query);
	if ($res){
		header('location:index.php?aksi=page&page=data_order_banyak');
	}
}
?>

<h3>Data Orderan Banyak</h3>
			<?php
			$query="select * from tbl_pembeli,tbl_trans where tbl_pembeli.id_pembeli=tbl_trans.id_pembeli and tbl_trans.jenis_trans='Order Banyak' order by id_trans DESC";
			$res=$con->query($query);
			...
			<table border="1" cellspacing="0" cellpadding="5" width="98%">
				...
				<tr><td>Nama Pembeli</td><td><?php echo $data['nama_lengkap']; ?></td></tr>
				...
				<tr><td>Pesanan</td><td><?php echo $data['pesanan']; ?></td></tr>
				<tr><td>Catatan Pembeli</td><td><?php echo $data['catatan']; ?></td></tr>
				...
				<a class="btn-red" title="Klik jika sudah dibayar" href="tbl_order_banyak.php?action=status_bayar&amp;id_trans=<?php echo $data['id_trans']; ?>">Sudah Dibayar</a>
				<?php } } ?>
